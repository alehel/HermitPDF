...
import { BatesIcon, PlusCircleIcon } from "@/components/Icons";
...
export default function BatesPage() {
  ...
  const {
    ...
    itemSelector: "[data-bates-item]",
    layout: "list",
    acceptDrag: (e) =>
      e.dataTransfer.types.includes("text/x-bates-index") ||
      e.dataTransfer.types.includes("Files"),
    getDropEffect: (e) =>
      e.dataTransfer.types.includes("Files") ? "copy" : "move",
    onDrop: (e, toIndex) => {
      const fromStr = e.dataTransfer.getData("text/x-bates-index");
      if (fromStr) {
            ...
            >
              {files.map((file, i) => (
                <FileCard
                  ...
                  extraProps={{ "data-bates-item": true } as React.HTMLAttributes<HTMLDivElement>}
                  dragHandle={{
                    onDragStart: (e) => {
                      e.dataTransfer.setData("text/x-bates-index", String(i));
